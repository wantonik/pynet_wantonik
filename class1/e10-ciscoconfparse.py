#!/usr/bin/env python
'''
Script using CiscoConfParse to find lines which does not use AES-SHA encryption method (based on the transform-set name).
Printing these lines along with their corresponding transform-set names.
'''
from ciscoconfparse import CiscoConfParse

file = open('cisco.txt', 'r')                 # Open text file in read mode
cisco_cfg = CiscoConfParse(file)              # Loading the text file to CCP module for parsing
print 'Checking if the text file got loaded by CCP module: \n', cisco_cfg        # checking if input file got loaded to CCP module
print
...
print 'Showing details of the lines which does not contain "AES-SHA" encryption method in parent-child format: '
